...
/// Represents a request to perform a card payment.
...
impl CardPaymentRequest {
    /// Constructs a new card payment request.
    pub fn new(
    ...
    /// Sets Apple Pay as the payment type via an encrypted token.
    ...
    /// Sets Google Pay as the payment type via an encrypted token.
    ...
    /// Sets the operation to be performed with a client.
    pub fn not_recurring(mut self) -> Self {
        self.recurring = Some(false);
        self
    }
...
/// Represents the response to a card payment operation.
